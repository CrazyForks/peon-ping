...
### Added
- **Grok Build adapter.** Grok's hook stdin is camelCase (`hookEventName`, `sessionId`) with snake_case event values (`session_start`, `stop`), so calling `peon.sh` directly stays silent. `adapters/grok.sh` and `adapters/grok.ps1` translate that envelope to CESP. When `~/.grok` exists, `install.sh` / `install.ps1` write `~/.grok/hooks/peon-ping.json` for `SessionStart`, `UserPromptSubmit`, `Stop`, `StopFailure`, `Notification`, subagent start/stop, `PostToolUseFailure`, and `PreCompact`. Noisy per-tool hooks stay unregistered. Session-end `Stop` fires (`reason: shutdown` / `channel_closed`) are treated as cleanup, not a completion chime. `run_terminal_command` failures map to Bash so `task.error` still plays. Stop-gate stdout is discarded so a chime cannot keep the agent working. PR #572, thanks @yrooogerg.
- **MiniMax T2A speech backend.** `scripts/tts-minimax.sh` and `tts-minimax.ps1` plug into the existing backend selector per `docs/adr/ADR-001-tts-backend-architecture.md`, resolving the regional endpoint from `PEON_MINIMAX_REGION` (`global_en` or `cn_zh`) and the model from `PEON_MINIMAX_MODEL`. The request body is assembled with `python3` so arbitrary speech text escapes safely, and the backend always exits 0, so a missing `MINIMAX_API_KEY`, `curl`, or `python3` degrades to silence rather than breaking a hook. PR #563, thanks @octo-patch.
- **OpenCode notifies when a question needs your input.** `question.asked` and `question.v2.asked` now fire an `input.required` notification, deduplicated by request id and released on reply or rejection, with a bounded pending set. PR #571, thanks @jmarin82.

### Fixed
- **The test suite could not start its own eval server on macOS CI, and had been reporting a missing file instead of saying so.** `http.server`'s `server_bind()` calls `socket.getfqdn()` on the bind address, a reverse-DNS lookup that blocks until the resolver gives up on a host with no reverse record for `127.0.0.1`. The server wrote neither its lockfile nor its port line until it returned, so all 34 tests in `eval-server.bats` plus the create-eval-approve end-to-end failed with an unexplained `FileNotFoundError`, while passing on every developer machine. `getfqdn()`'s result only ever populated `server_name`, a CGI variable this server does not emit, so the bind now goes through `TCPServer` directly. This is what had kept main red since v2.36.0.
- **Click-to-focus works in local tmux sessions, and on kitty and alacritty at all.** The hosting terminal was identified from the hook's own environment (`ITERM_SESSION_ID`, `GHOSTTY_RESOURCES_DIR`, `WARP_IS_LOCAL_SHELL_SESSION`), but a tmux pane's environment is frozen when the pane is created and can never be rewritten. Inside tmux that named the emulator the session was *first* attached from, forever, so reopening the terminal or attaching from a different one activated an app that was not showing the session. kitty and alacritty set none of those, so the bundle id came out empty, nothing was raised, and the notification looked dead. `scripts/local-focus.sh` now asks tmux which client is attached at click time and walks that client's process ancestry to the hosting terminal, whatever it happens to be. PR #565, thanks @lukewang1024.
- **Relayed remote-SSH notifications are clickable.** The relay POSTed only `{title,message,color}`, dropping every piece of click-to-focus context, and the overlay registered no click handler for relayed notifications at all. `peon.sh` now embeds a validated `focus` object in `POST /notify`, `relay.sh` whitelist-validates every field and drops the object entirely on any mismatch, and `scripts/ssh-focus.sh` raises the local terminal tab hosting the ssh session and switches the remote tmux to the agent's pane. The relay also scrubs its own ambient `TMUX` so it cannot fabricate a local target. PR #562, thanks @lukewang1024.
- **The Windows hook could hang forever on stdin.** A host can open the hook's stdin pipe and never close the write end, and the synchronous `ReadToEnd()` then parked the pipeline thread for the life of the process. The 8-second safety timer could not rescue it: `Register-ObjectEvent` runs its action on that same blocked thread. The read is now bounded at 2 seconds, matching the `timeout 2 cat` guard `peon.sh` has had since #445, and a timeout falls through to the existing no-input guard. PR #570, thanks @wilyan09007. Closes #568, reported and root-caused by @tomzorz.
- **ntfy auth headers with a space in the token were malformed.** The header was built as a string, `auth_header="-H \"Authorization: Bearer $TOKEN\""`, and then expanded unquoted, so the shell word-split it on any space in the token and the quote characters arrived literally. It is now a bash array. `peon mobile test` also reports real success or failure instead of always exiting 0. PR #576, thanks @SikongJueluo.
- **`peon relay --status` reports the port the daemon actually bound.** It previously echoed whatever `RELAY_PORT` resolved to inside the status invocation, which is the caller's environment or the hardcoded default, so a healthy daemon started on a non-default port was reported with a stale one whenever the check ran without the original `--port` or `PEON_RELAY_PORT`. The bound port is now persisted alongside the pid. PR #578, thanks @yphillip.
- **OpenCode fired `SessionStart` twice and `UserPromptSubmit` on every status change.** A 100ms timer fired a greeting on plugin load in addition to the real `session.start` event, and any `busy`/`running` status transition re-fired the prompt sound for a session already working. Both are debounced. PR #566, thanks @GuillaumeAmat.
- **OpenCode wrote its tab-title escape sequence into piped output.** `setTabTitle()` emitted the OSC sequence unconditionally, so it landed in whatever consumed opencode's stdout when it was redirected. It now writes only when stdout is a TTY. PR #580, thanks @ImBIOS.
- **All five bundled skills had their declared visibility silently ignored.** The frontmatter key is `user-invocable`; every skill spelled it `user_invocable`, which Claude Code does not read, so `peon-ping-config` was never actually hidden from the slash-command list. PR #564, thanks @plaffitt.

### Internal
- The opencode adapter's vitest suite now runs in CI. It existed, it passed, and nothing had ever executed it, so 58 tests gated nothing while three separate PRs edited that one file.
- Two races in `eval-server.bats` are fixed, both masked until now by the startup failure above. The busy-state tests waited on an SSE line to decide a job was in flight, but `curl` buffers the event stream when its stdout is a file, so `started` could surface only once `done` pushed it through. The concurrent-reroll test assumed ten curls land simultaneously, when the last can arrive after a job that finishes in the time it takes to append one byte per wav. Both now hold the job open deliberately, and both were verified by mutation.

## v2.36.0 (2026-08-09)

### Added
- **`peon create` and `peon eval`: a sound pack does not exist until a human has heard it and approved it.** Generating a pack used to end with files on disk that you had to go find and audition yourself, which meant a bad take could ship simply because nobody listened. Creation now always produces a *draft*, stamped `x_openpeon_draft: true` in its manifest and written to `~/.peon-ping/drafts/<name>/`, and `peon packs use` refuses to activate one. `peon create` (interactive, or `--name/--flavor/--vibe`) authors and renders the draft, then opens the gate automatically; `peon eval <name-or-path>` opens it on demand, and on an already-approved pack re-drafts it so you can take another pass. The gate itself is a local page served from `127.0.0.1`: every sound laid out by CESP category with a play button, "Reroll this sound" and "Reroll whole pack" each taking a plain-language note on *why* ("too harsh, want something softer"), and an Approve button that is the only way a draft becomes a pack. Approving strips the draft stamp, moves the pack to `~/.peon-ping/packs/<name>`, optionally installs it, and shuts the server down.
- **Rerolls run on your own Claude Code and your own ElevenLabs key.** A reroll hands your caption to `claude -p` with the bundled `peon-ping-remix` skill, which rewrites that sound's generation prompt to honor the note, re-renders it, and appends the caption plus the old and new prompts to the pack's `eval-log.json`, so an approved pack carries the record of how it got there. Nothing is billed to anyone else and no audio leaves your machine except the render request you triggered. The ElevenLabs key is read from `ELEVENLABS_API_KEY`, then `~/.config/peon-ping/credentials`.
- **`scripts/pack-render.py`**, a stdlib-only single-sound renderer with a silence gate: a take that comes back below -50 dB is re-rendered once with a strengthened prompt and then reported as a failure rather than shipped, and renders are written to a temporary file and moved into place only on success, so a failed or silent render can never destroy the take you already had.

### Security
- **The eval server is token-gated.** It mints a per-session token, stores it in the draft's lockfile, and requires it on every `/api/*` call, alongside `Host`, `Origin`, and `Content-Type` validation. Without this, any web page you visited while an eval session was open could have approved and installed a draft, or driven a `claude -p` child on your machine, using a cross-origin request that needs no preflight. Sound files are served only from within the draft's own `sounds/` directory, with symlink and traversal containment enforced against the resolved real path.
- **Pack names from a manifest are validated before they are used as a path.** A manifest claiming `"name": "../victim/pwned"` previously relocated the pack outside the approved root on approval; the name is now pattern-checked, the destination is containment-checked, and the manifest is normalized to match the directory it actually lands in.

### Fixed
- **A reroll that could not do its work no longer reports success.** A headless `claude -p` that hits a permission boundary explains itself and exits `0`, so the server reported `done` and the page reloaded the unchanged audio, telling you your note had been honored when nothing had happened. This affected every install that had not opted into auto-approving edits. The spawned agent is now given the draft directory and the peon-ping scripts directory explicitly, and the server verifies the files actually changed before reporting `done`, surfacing the agent's own explanation on a failure instead.
...
### Added
- **Standard notification banners focus the exact terminal window on click.** Notification Center banners passed only `-activate <bundle-id>` to terminal-notifier, so with several iTerm2 or IDE windows open, clicking a banner raised whichever window was focused last, not the one that pinged. The overlay already solved this for iTerm2 sessions; standard mode now gets the same precision, selecting the iTerm2 tab/session whose tty matches and raising the VS Code family window that has the workspace open (via `open -b`). PR #556, thanks @KyleOps.
- **Overlay renders a subtitle line when one is supplied.** `mac-overlay.js` accepted a subtitle argument populated by `notify.sh` but never drew it; the overlay now stacks the title over the subtitle when present, keeping the single centered line otherwise. Part of PR #555, thanks @lukewang1024.

### Fixed
- **WSL2 MediaPlayer playback no longer cut to silence on Windows 24H2 builds.** On build 26200 `NaturalDuration` is not resolved right after `Open()` (`HasTimeSpan` is False, so `TimeSpan` reads 00:00:00), so the old wait loop ran zero iterations and closed the player within milliseconds of `Play()`. Hook events and `peon preview` were silent while the `peon update` self-test still worked. Playback now mirrors the native Windows backend: register `MediaOpened`/`MediaFailed`, pump the WPF dispatcher until the media opens, then wait on the resolved duration (with a short fallback). Reported and root-caused by @Antreesy. Closes #558.
- **Codex sessions are never interrupted by a missing adapter, and reinstall/uninstall preserve unrelated Codex config.** Codex stable hooks invoked the adapter by absolute path, so a removed or moved runtime made events like `UserPromptSubmit` exit 127 and interrupt the conversation. Hook commands now fail open on both Unix and Windows. The inline TOML editing moved into a dedicated `scripts/codex-config.py` that removes only peon-ping's own hook sections for the current install root and preserves `[hooks.state]`, `[[skills.config]]`, sibling handlers, multiline TOML, CRLF, and symlinked install paths, writing atomically. Regression from #549 in v2.34.0. PR #557, thanks @zenvor.
- **Notifications fire from background tmux panes, and clicking the overlay jumps to the agent's own pane.** `terminal_is_focused` did not account for tmux, so an agent working in a background tmux window or pane (or a detached session) was treated as focused and its notification suppressed. It is now tmux-pane aware. On click, the overlay also runs a `tmux switch-client/select-window/select-pane` command targeting the agent's pane, after the hosting terminal window is raised. Part of PR #555, thanks @lukewang1024.
- **Warp notifications focus the exact tab across Spaces.** `activateWithOptions()` returns success from the overlay's accessory-policy process but does not cross macOS Spaces, so clicking a notification never brought Warp forward when it lived on another desktop. Warp's per-session deep link (`WARP_FOCUS_URL`) is now threaded to the overlay click handler, which opens it to switch app, Space, and the exact tab, falling back to AppleScript activate on older Warp. PR #551, thanks @csatib02. Closes #547.
- **Uninstaller handles symlinked rc files and removes every hook handler.** `sed -i` aborts on a symlinked `~/.zshrc` (the classic dotfiles layout), so rc cleanup failed; it now edits through the symlink while preserving it. Hook-registration removal was generalized from `hook-handle-use` to every `hook-handle-*` handler the installer registers, including the rename hook. PR #554, thanks @paskal.
- **Uninstaller is safe to run through a pipe.** Running `curl ... | bash` against `uninstall.sh` tripped `set -u` on `BASH_SOURCE[0]` or resolved the script path incorrectly; piped execution is now detected and handled, with safer path resolution. PR #559, thanks @piersjones.
- **User prompt text is no longer written to hook logs.** The `UserPromptSubmit` handlers logged the first 80 characters of every non-command prompt to a plaintext log, which could capture pasted credentials; they now log only the prompt length, and log files are created `0600`. PR #553, thanks @paskal.
- **Nix installs build the Swift helpers.** The Nix package's `installPhase` copied only the shell and JS helpers, so `meeting-detect` and `peon-play` were never compiled and `meeting_detect: true` silently no-opped on Nix. The flake now builds both Swift helpers. PR #552, thanks @starcraft66. Ref #539.
